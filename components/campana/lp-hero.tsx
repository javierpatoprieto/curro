...
import { btnBosqueLg } from "@/components/marketing/ui";
import type { Gremio } from "@/lib/gremios";

/**
 * Hero de la landing de campaña. Message-match con los anuncios: mismo gancho
 * (no pierdas clientes por no coger el teléfono), un único CTA a `/registro` y
 * el subtítulo específico del gremio. Reutiliza la tarjeta de producto del hero
 * de la home (imagen + "Curro está atendiendo" + onda).
 */
export function LpHero({ gremio }: { gremio: Gremio }) {
  const { ejemplo } = gremio;
  return (
    <section className="bg-nieve">
      <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:py-20">
        <div>
          ...
          <h1 className="titular mt-6 text-5xl text-bosque sm:text-6xl lg:text-[4.4rem]">
            No pierdas clientes por no coger{" "}
          ...
          <p className="mt-7 text-sm text-bosque-soft">
            Sin permanencia · Prueba gratis 7 días · Listo en 10 min
          </p>
        </div>
